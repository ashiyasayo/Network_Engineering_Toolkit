...
macOS/Linux Agent 使用權限為 `0600` 的 Unix domain socket；Windows Agent IPC 使用 Named Pipe 與相同 bounded framing。`nettool-gui` 現提供 loopback-only localhost Dashboard，`nettool-desktop` 則以 Tauri 2 提供正式原生 WebView 殼層，啟動並管理 Agent 與 GUI backend，仍不直接執行網路或特權操作。Linux 已提供 `nettool-helper` root service、Unix peer authentication、Safe Apply、Hosts、NetworkManager Ethernet、PCI driver 與 Huge Page executors，以及 systemd 安裝資產；macOS Unix helper 已接上 `networksetup` platform executor 與同一 Safe Apply/IPC 流程，Windows helper 已接上 Named Pipe token SID allowlist、`netsh` executor 與 Safe Apply。三平台 installer staging 資產已提供；正式簽章、notarization、Windows ACL 與硬體實機驗收仍需各平台 release runner。
