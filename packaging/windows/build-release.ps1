...
cargo build --manifest-path (Join-Path $root "Cargo.toml") --release -p nettool-desktop -p nettool-agent -p nettool-gui -p nettool-dataplane
New-Item -ItemType Directory -Force -Path (Join-Path $root $OutputDirectory) | Out-Null
foreach ($name in @("nettool-desktop.exe", "nettool-agent.exe", "nettool-gui.exe", "nettool-dataplane.exe")) {
