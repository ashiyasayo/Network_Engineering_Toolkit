...
> 本文件用來回答三個問題：每個程序負責什麼、資料如何流動，以及哪些條件通過後才可以執行或宣稱成功。
>
> 快速開始請參閱 [README](../README.md)，命令與輸出格式請參閱 [CLI Reference](CLI_REFERENCE.md)，規格與驗證證據請參閱 [需求追蹤矩陣](REQUIREMENT_TRACEABILITY.md)。
...
| 想了解的問題 | 建議閱讀 | 主要圖表 |
| --- | --- | --- |
| 使用者操作會經過哪些程序？ | [1. 程序責任與分層](#1-程序責任與分層) | 系統邊界圖 |
| 為什麼 GUI 不能直接修改網路？ | [2. 控制平面與信任邊界](#2-控制平面與信任邊界)、[5. Identity、Safe Apply 與 Helper](#5-identitysafe-apply-與-helper) | 特權操作時序圖、Safe Apply 流程圖 |
| `speed.run` 如何管理雙端測試？ | [6. Speed session 與 Node control](#6-speed-session-與-node-control) | Session lifecycle 圖 |
| Backend 何時算可用？ | [4. Capability、implementation 與 runtime gate](#4-capabilityimplementation-與-runtime-gate) | 三層 gate 圖 |
| 封包如何變成分析結果？ | [9. Packet data plane](#9-packet-data-plane) | Packet pipeline 圖 |
| 發行包如何產生與驗證？ | [10. 發行與部署](#10-發行與部署) | Release pipeline 圖 |

## 系統全貌

```mermaid
flowchart LR
    User[使用者] --> CLI[nettool CLI]
    User --> Desktop[nettool-desktop<br/>Tauri 2]

    subgraph UserSpace[使用者權限程序]
        CLI --> Client[agent-client]
        Desktop -.管理程序生命週期.-> Agent
        Desktop -.啟動 loopback Dashboard.-> GUI[nettool-gui]
        GUI --> Client
    end

    Client -->|4-byte BE + Protobuf<br/>max 1 MiB| Agent[nettool-agent<br/>runtime authority]
    Agent --> Store[(SQLite metadata)]
    Agent -->|probe / capture worker| DataPlane[nettool-dataplane]
    Agent -->|authenticated local transport| Helper[nettool-helper<br/>privileged boundary]
    Agent -->|optional TCP + mTLS 1.3| Node[Remote nettool-agent]
    Helper -->|fixed argv / platform API| OS[OS network state]
    DataPlane -->|NIC / driver / backend| NIC[Network interface]
```

圖表解讀：

- CLI 與 GUI 都透過 `agent-client` 呼叫 Agent；Agent 統一處理 Action、metadata、session 與 dataplane worker。
- `nettool-desktop` 只管理 Tauri WebView 與 Agent/GUI 程序生命週期，不直接操作網路。
- 需要修改 IP、DNS、Routes、Hosts 或 driver binding 時，Agent 必須呼叫 authenticated Helper；Helper 再使用固定參數與平台 API。
- `nettool-dataplane` 負責 probe、socket/backend worker、封包分析與硬體 evidence，但不取代 Agent 的 trust 或權限判定。
...
```mermaid
sequenceDiagram
    participant Caller as CLI / GUI
    participant Agent as nettool-agent
    participant Helper as nettool-helper
    participant OS as OS network API

    Caller->>Agent: Action request
    Note over Caller,Agent: 4-byte big-endian length + Protobuf envelope<br/>frame 上限 1 MiB
    Agent->>Agent: 驗證 schema、permission、dry_run 與 operation ID
    alt read-only action
        Agent-->>Caller: typed result / stable error
    else privileged action
        Agent->>Helper: authenticated request
        Helper->>Helper: peer identity + exact allowlist
        Helper->>OS: fixed argv / typed platform call
        OS-->>Helper: snapshot / apply result
        Helper-->>Agent: verified result / rollback state
        Agent-->>Caller: typed result / stable error
    end
```

圖表解讀：CLI/GUI 不直接碰 OS network API；Agent 先做 schema、權限與 operation 驗證，只有明確的 privileged action 才能進入 Helper。Helper 回傳前必須完成 read-back verify 或保存可恢復的 rollback state。

控制流程的不可繞過規則：

- Unix Helper 從 kernel peer credentials 取得 UID/PID；Windows Helper 從 Named Pipe client handle 取得 Token SID。身份不接受 caller 在 JSON 內自行宣告。
- Agent IPC 與 Helper transport 都先驗證長度，再配置 payload；user-only socket、Named Pipe SID allowlist 與 exact action allowlist 形成不同層次的邊界。
- `dry_run` 會先完成相同的 JSON/schema validation；read-only action 回傳 plan 與 payload hash，privileged action 將旗標帶到 Helper，但不執行副作用。
- 每個長時間 IPC connection 使用獨立 task，避免 `speed.run` 阻塞後續 `speed.cancel`；取消仍須走對應的 Node control authorization。

## 3. Runtime module seams
...
`dry_run` 旗標由 runtime 實際處理：非 privileged action 回傳 payload hash、permission metadata 與「未執行副作用」plan；需要 privileged Helper 的 action 則將旗標傳入 Helper dry-run path。

## 4. Capability、implementation 與 runtime gate
...
```mermaid
flowchart TB
    Surface[Platform / kernel surface<br/>系統表面存在]
    Impl[Implementation availability<br/>程式碼 + native SDK 已連結]
    Runtime[Runtime gate<br/>interface / driver / queue / permission / resource]
    Result{可執行或可認證？}
    Reject[維持 unavailable / warning / failure]
    Surface --> Impl --> Runtime --> Result
    Surface -.僅代表 surface.-> Reject
    Impl -.未連結.-> Reject
    Runtime -.未通過.-> Reject
```

圖表解讀：platform surface 只代表系統表面存在，implementation 代表程式碼與 native SDK 已連結，runtime gate 才代表目前介面、driver、queue、權限與資源檢查通過；任何一層失敗都不能產生 synthetic result。

## 5. Identity、Safe Apply 與 Helper

### Identity 與資料儲存

Node identity 由獨立 Identity crate 管理，production store 直接使用平台 native credential backend：

- macOS 使用 Keychain、Windows 使用 Credential Manager、Linux 使用 Secret Service；沒有一般檔案或 SQLite fallback。
- 首次啟動以 CSPRNG 建立非零 128-bit Node ID，產生 PKCS#8 asymmetric key 與含 SAN 的 certificate。
- credential blob 使用 bounded、versioned binary envelope；每次載入都驗證 header/length、X.509 DER、PKCS#8 DER，以及 certificate public key/private key 是否相符。
- Agent 在開啟 IPC listener 前先載入 identity material；store 鎖定、損壞或不可用時，Agent 必須啟動失敗。

### Safe Apply lifecycle

```mermaid
flowchart LR
    Desired[typed Desired State<br/>deny unknown fields] --> Validate[validate schema<br/>operation ID / deadline]
    Validate --> Snapshot[read and persist<br/>helper-owned snapshot]
    Snapshot --> Apply[fixed argv / platform API]
    Apply --> Verify[逐欄 read-back verify]
    Verify --> Confirm{confirm before deadline?}
    Confirm -->|yes| Commit[mark completed<br/>release snapshot]
    Confirm -->|no / Agent crash| Rollback[restore snapshot<br/>mark rolled back]
    Apply -->|error| Rollback
```

圖表解讀：snapshot、deadline 與 rollback 狀態屬於 Helper，不屬於易中斷的 Agent process。寫入使用暫存檔、`fsync` 與 atomic rename；Unix 另外同步 parent directory，因此 Agent 終止或重啟後仍可恢復逾期 operation。

Safe Apply 的狀態與恢復規則如下：

- deadline 與 snapshot ID 由 helper-owned state 保存；即使 Agent 終止，Helper 重啟後仍可查出 pending operation 並依 deadline rollback。
- 寫入流程使用暫存檔、`fsync` 與 atomic rename；Unix 另外同步 parent directory，補足 crash durability。
- Desired state 是拒絕未知欄位的 typed schema；相同 operation request 可冪等重送。
- 不同 payload 重用 operation ID，或在 deadline 後 confirm，都必須拒絕。

### 平台 adapter

平台 adapter 只負責平台差異；generic `PlatformNetworkExecutor` 統一 fixed-argv、typed snapshot、verify 與 restore。

- **Linux**：使用 absolute `nmcli` path 呼叫 NetworkManager，不依賴 privileged process 的 `PATH`，也不經 shell。Snapshot 以 active connection UUID 讀取 IPv4/IPv6、DNS、routes 與 MTU；modify/activate 後逐欄讀回驗證，rollback 由 snapshot 還原並重新 activate。
- **macOS**：使用 `networksetup` reader/executor，接到 shared fail-closed Safe Apply wiring。
- **Windows**：state reader 使用固定 PowerShell JSON query，apply 使用 `netsh` fixed-argv builder；query 輸出固定為無 BOM UTF-8，Named Pipe 以 token SID allowlist 建立 authenticated boundary。
- **發行 gate**：GitHub Actions release workflow 已加入 prerelease/stable gate；只有 secrets 齊全才執行 Apple/Windows/Linux artifact signing。實際憑證、notarization、平台 ACL/rollback 與完整 installer 驗收仍待外部條件，因此目前不標示 production-ready。
...
## 6. Speed session 與 Node control
...
stateDiagram-v2
    [*] --> NEGOTIATE
    NEGOTIATE --> PREPARE: capability / protocol valid
    PREPARE --> READY: local + remote endpoint ready
    READY --> WARMUP: scheduled Start
    WARMUP --> MEASURE
    MEASURE --> COOLDOWN
    COOLDOWN --> FINALIZE
    FINALIZE --> RESULT: immutable result + checksum
    PREPARE --> CANCELED: Stop / timeout
    READY --> CANCELED: Stop / timeout
    WARMUP --> CANCELED: Stop
    MEASURE --> CANCELED: Stop
    FINALIZE --> CANCELED: Stop
    PREPARE --> FAILED: fatal error
    READY --> FAILED: fatal error
    WARMUP --> FAILED: fatal error
    MEASURE --> FAILED: fatal error
    FINALIZE --> FAILED: fatal error
```

圖表解讀：正常完成一定經過 `FINALIZE` 才保存 immutable result；`Stop` 只適用於尚未完成的 session，不能把已完成結果誤標為 Canceled。

### Speed engine 與 wire format
...
### Request validation 與 endpoint prepare
...
```mermaid
flowchart LR
    Agent[Agent] --> TCP[TCP control connection]
    TCP --> TLS[TLS 1.3 + mutual authentication]
    TLS --> Hello[NTCP Hello<br/>Node ID + protocol version]
    Hello --> Cap[Capability negotiation]
    Cap --> Prepare[Prepare / Start / Stop / ResultQuery]
    Prepare --> Coordinator[Agent-owned<br/>SessionCoordinator]
```

圖表解讀：Node control 只協調雙端 session，不承載測試封包；實際 TCP/UDP data plane 仍須另行通過 endpoint、session、stream 與 authorization tag 驗證。

### Pairing、session storage 與 prepare
...
### Data-plane authorization 與 result
...
## 7. Benchmark、分析與 certification

### 固定 phase order

```mermaid
flowchart LR
    Env[Environment] --> NICPhase[NIC]
    NICPhase --> RX[RX]
    RX --> TX[TX]
    TX --> Bi[Bidirectional]
    Bi --> PacketMatrix[Packet Matrix]
    PacketMatrix --> FlowMatrix[Flow Matrix]
    FlowMatrix --> Duration[Duration]
    Duration --> Analysis[Analysis]
    Analysis --> ResultPhase[Result]
```

圖表解讀：每個 phase 都產生 bounded evidence；runner 不自行 sleep 或產生 throughput，所有量測數值必須來自真正的 backend executor。Cancel 只在 phase boundary 生效，已完成 phase 保留，其餘明確標記 skipped。
...
### Application 與 Storage boundary
...
### Domain payload 與 unsafe boundary
...
## 8. NIC probe 與 backend

### Probe 到 worker 的資料流

```mermaid
flowchart LR
    Probe[唯讀 NIC probe] --> Snapshot[capability snapshot]
    Snapshot --> Preflight[backend preflight]
    Preflight --> Queue[QueuePlan<br/>NUMA / CPU / queue ownership]
    Queue --> Worker[backend worker]
    Preflight -->|任一 gate 失敗| Unavailable[unavailable / warning / failure]
```

圖表解讀：probe 只提供觀察到的系統證據；preflight 將證據與 implementation/runtime gate 合併判定，通過後才允許 queue planner 交付給 native worker。

### NIC probe 與 queue planner

- **Linux**：依 sysfs device path 回報 `bus_type`（`usb`、`pci` 或 `unknown`）；只有合法 PCI BDF 才填入 `pci_address`，不把 USB、virtual、缺少 device 或 malformed path 猜測成 PCI。
- **Windows**：使用固定 PowerShell JSON query 與 UTF-8 stdout，以同一張 adapter 的 `ifIndex` 關聯 `ip_addresses`；嚴格 UTF-8/JSON 解析保留中文介面名稱、正確 IP 與 IPv6 link-local scope suffix。
- **共同限制**：probe 與 IP metadata 都是唯讀資訊，不等於 hot-plug、auto-provision 或實體硬體驗收。
- **DPDK preflight**：每次使用最新 capability snapshot，檢查 runtime、PCI device、userspace driver、RX/TX queues、NUMA locality、Huge Pages 與 CPU affinity。
- **Management NIC protection**：Linux dataplane 從 default route 唯讀解析 management interface，再映射到最新 NIC PCI evidence；若與目標相同，由 `MANAGEMENT_NIC_PROTECTION` gate 拒絕，其他平台或無法解析時不猜測。
- **Queue planner**：輸出同 NUMA 的 one-queue/one-worker contiguous ownership；`QueuePlan::validate` 在 native orchestration 前再次拒絕重複 CPU 或不連續 queue ID。一般模式可 degraded 執行，但任何 warning 都使結果不可認證；certification mode 將 NUMA、driver 與 Huge Page 缺失提升為 failure。
...
```mermaid
flowchart LR
    Source[BurstSource<br/>NIC / offline capture] --> Counters[RX counters<br/>worker-local]
    Counters --> Filter[filter<br/>protocol / IP / port]
    Filter --> Parser[bounds-checked parser<br/>Ethernet / VLAN / IP / TCP / UDP]
    Parser --> Capture[bounded capture branch<br/>PCAP / PCAPNG]
    Parser --> Flow[worker-local flow table<br/>max 1,000,000]
    Flow --> TCP[TCP analyzer]
    Flow --> Aggregate[low-frequency aggregator]
    TCP --> Aggregate
    Aggregate --> Result[pps / bps / loss / jitter / confidence]
```

圖表解讀：`BurstSource` 是 backend 與 worker 的 ownership boundary；hot path 只處理 bounded borrowed data，capture、flow 與 analyzer 的結果最後才由低頻 aggregator 合併。
...
```mermaid
flowchart LR
    Source[version tag vX.Y.Z] --> Build[build five binaries]
    Build --> Stage[fixed allowlist staging]
    Stage --> Bundle[Tauri / portable bundle]
    Bundle --> Smoke[platform package smoke test]
    Smoke --> Pre[GitHub prerelease]
    Pre --> Gate{stable workflow<br/>secrets + signing gate}
    Gate -->|all conditions pass| Stable[signed stable release]
    Gate -->|missing / invalid| Stop[fail closed<br/>no unsigned stable release]
```

圖表解讀：tag push 只建立 prerelease；stable 必須由 `workflow_dispatch` 明確選擇，並通過平台簽章與 notarization gate。這讓測試包可以先驗證，而不會在憑證不完整時誤產生正式未簽章版本。
...
下列狀態不能從「程式碼存在」、「cargo check 通過」或「單一 runner 通過」推論而來：
...
- [Agent runtime](../apps/agent/src/main.rs) 與 [Action dispatch](../apps/agent/src/action_dispatch.rs)
- [Desktop process lifecycle](../apps/desktop/src/main.rs) 與 [Tauri configuration](../apps/desktop/tauri.conf.json)
- [Helper shared network executor](../crates/helper-core/src/platform_network.rs)
- [Windows network adapter](../crates/helper-core/src/platform_network_windows.rs)
- [DPDK backend](../crates/backend-dpdk/src/lib.rs)、[AF_XDP backend](../crates/backend-af-xdp/src/lib.rs) 與 [RIO backend](../crates/backend-rio/src/lib.rs)
- [Dataplane CLI](../apps/dataplane/src/main.rs)
- [Release workflow](../.github/workflows/release.yml) 與 [發行說明](../packaging/README.md)
