...
探測平台、CPU、NUMA、Huge Page、NIC、IP address、queue、驅動、NIC bus type 與資料平面能力，不修改系統。Linux 依 sysfs device path 判定 `usb`、`pci` 或 `unknown`；USB 或未知介面不會被填入 PCI address。Windows 以固定 PowerShell JSON 查詢取得介面名稱與 `ip_addresses`，先設定 UTF-8 stdout，再以同一介面的 `ifIndex` 查詢位址；查詢輸出無法驗證時會 fail closed。這是唯讀分類，不代表已完成 USB hot-plug、auto-provision 或真實硬體驗收。
